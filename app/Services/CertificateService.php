...
class CertificateService
{
     ...
     */
    public static function extractToPem(string $filePath, string $password): array
    {
        try {
            if (!Storage::exists($filePath)) {
            ...
            $certData = openssl_x509_parse($certs['cert']);
            $ruc = $certData['subject']['serialNumber'] ?? $certData['subject']['CN'] ?? 'unknown';
            $timestamp = time();

            // Guardar certificado en formato PEM
            $certFilename = "cert_{$ruc}_{$timestamp}.pem";
            $certPath = "keys/{$certFilename}";
            Storage::put($certPath, $certs['cert']);

            // Guardar clave privada en formato PEM
            $keyFilename = "key_{$ruc}_{$timestamp}.pem";
