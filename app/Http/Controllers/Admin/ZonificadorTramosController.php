...
class ZonificadorTramosController extends Controller
{
    ...
    public function index()
    {
        ...
        // Obtener todos los usuarios activos para asignar rutas
        $usuarios = User::where('status', 1)
            ->with('persona:id,nombres,ape_pat,ape_mat')
            ->get(['id', 'codigo', 'persona_id', 'name']);

        // Cargar zonas con sus sucursales
        $zonas = Zona::with('sucursales')->get();

        return view('admin.zonificador-tramos.index', compact('jccs', 'asesores', 'analistas', 'zonas', 'usuarios'));
    }
    ...
    public function getData(Request $request)
    {
        try {
            ...
            // ==========================================
            // NOTA: Solo traemos cuotas NO PAGADAS (estados 0,1,3) para el zonificador
            // Las moras se calculan posteriormente con todas las cuotas del préstamo
            $query = Cuota::query()
            ...
            $resultado = $cuotasValidas->groupBy(function ($cuota) {
                ...
                // Calcular moras de TODAS las cuotas (incluyendo pagadas con moras pendientes)
                foreach ($todasCuotas as $cuota) {
                    // Cargar moras si no están cargadas
                    if (!$cuota->relationLoaded('moras')) {
                        $cuota->load('moras');
                    }
                ...
                // Buscar la PRIMERA cuota NO PAGADA (sin importar si ya venció)
                // Esto representa el inicio real de la deuda del cliente
                foreach ($todasLasCuotasOrdenadas as $cuota) {
                    $estado = $cuota->estado;
                    $estadoValor = $estado instanceof \BackedEnum ? $estado->value : (is_numeric($estado) ? (int) $estado : null);
                    $noEstaPagada = in_array($estadoValor, [0, 1, 3]); // PENDIENTE, PARCIAL, VENCIDO
                ...
                if ($primeraCuotaVencida) {
                    ...
                    // Calcular días de atraso solo si la fecha ya pasó
                    if ($fechaVencimiento->lt($fechaActual)) {
                        $diasAtraso = $fechaActual->diffInDays($fechaVencimiento);
                    } else {
                        // Si la cuota aún no vence, días de atraso = 0
                        $diasAtraso = 0;
                    }
                ...
                // Calcular estado crediticio
                $estadoCredito = $this->calcularEstadoCredito($prestamo, $cuotasVencidas, $moraTotal, $todasCuotas);
                ...
                return [
                    ...
                    'estado' => $estadoCredito, // Nuevo campo de estado crediticio
                ];
            })->values();
    ...
    /**
     * Calcular estado crediticio del cliente
     */
    private function calcularEstadoCredito($prestamo, $cuotasVencidas, $moraTotal, $todasCuotas)
    {
        // Verificar si el crédito está activo (prestamo.estado == 1)
        $activo = ($prestamo->estado == 1);
        
        // Si no hay cuotas vencidas ni mora
        if ($cuotasVencidas == 0 && $moraTotal == 0) {
            return $activo ? 'ACTIVO' : 'INACTIVO';
        }
        
        // Si hay mora pero no cuotas vencidas
        if ($cuotasVencidas == 0 && $moraTotal > 0) {
            return $activo ? 'EN MORA/ACTIVA' : 'EN MORA/INACTIVA';
        }
        
        // Si hay cuotas vencidas (crédito vencido)
        if ($cuotasVencidas > 0) {
            // Determinar si el crédito está completamente vencido
            $totalCuotas = $todasCuotas->count();
            $cuotasPagadas = $todasCuotas->where('estado', 2)->count();
            
            // Si todas las cuotas están vencidas o la mayoría
            if ($cuotasVencidas >= ($totalCuotas - $cuotasPagadas)) {
                return $activo ? 'CREDITO VENCIDO/ACTIVO' : 'CREDITO VENCIDO/INACTIVO';
            } else {
                return $activo ? 'EN MORA/ACTIVA' : 'EN MORA/INACTIVA';
            }
        }
